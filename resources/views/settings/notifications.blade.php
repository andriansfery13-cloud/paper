...
@section('header', 'Pengaturan Notifikasi')

@section('content')
    <div class="max-w-4xl mx-auto space-y-6">
        <div class="bg-white rounded-xl shadow-sm border p-6">
            <h3 class="text-lg font-semibold text-gray-900 mb-6">Konfigurasi Notifikasi</h3>
            ...
            <form action="{{ route('settings.notifications.update') }}" method="POST">
                    ...
                    <div class="flex items-center justify-between mb-4">
                        ...
                        <div class="flex items-center">
                            <label class="relative inline-flex items-center cursor-pointer">
                                <input type="checkbox" name="whatsapp_enabled" value="1" class="sr-only peer" {{ ($tenant->settings['notifications']['whatsapp']['enabled'] ?? false) ? 'checked' : '' }}>
                                <div
                    ...
                    <div class="grid grid-cols-1 gap-4 p-4 border rounded-lg bg-gray-50">
                        <div>
                            ...
                            <input type="text" name="whatsapp_token"
                                value="{{ $tenant->settings['notifications']['whatsapp']['token'] ?? '' }}"
                                class="w-full px-3 py-2 border rounded-lg focus:ring-2 focus:ring-green-500 focus:border-green-500"
                                placeholder="Masukkan token Fonnte">
                            <p class="mt-1 text-xs text-gray-500">Dapatkan token API Anda dari dashboard Fonnte.</p>
                        </div>
                    </div>
                </div>
                ...
                <div class="mb-8">
                    <div class="flex items-center justify-between mb-4">
                        ...
                        <div class="flex items-center">
                            <label class="relative inline-flex items-center cursor-pointer">
                                <input type="checkbox" name="telegram_enabled" value="1" class="sr-only peer" {{ ($tenant->settings['notifications']['telegram']['enabled'] ?? false) ? 'checked' : '' }}>
                                <div
                    ...
                    <div class="grid grid-cols-1 gap-4 p-4 border rounded-lg bg-gray-50">
                        <div>
                            ...
                            <input type="text" name="telegram_bot_token"
                                value="{{ $tenant->settings['notifications']['telegram']['bot_token'] ?? '' }}"
                        ...
                        <div>
                            ...
                            <input type="text" name="telegram_chat_id"
                                value="{{ $tenant->settings['notifications']['telegram']['chat_id'] ?? '' }}"
                                class="w-full px-3 py-2 border rounded-lg focus:ring-2 focus:ring-blue-500 focus:border-blue-500"
                                placeholder="Chat ID Admin">
                            <p class="mt-1 text-xs text-gray-500">ID chat administrator untuk menerima notifikasi sistem.
                            </p>
                        </div>
                    </div>
                </div>
