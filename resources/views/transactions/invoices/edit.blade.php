...
@section('content')
    <form action="{{ route('invoices.update', $invoice) }}" method="POST" x-data="invoiceForm()" class="space-y-6">
            ...
            <div class="grid grid-cols-3 gap-6">
                <div>
                    ...
                    <input type="text" name="invoice_number" value="{{ old('invoice_number', $invoice->invoice_number) }}" required
                        class="w-full px-4 py-2 border rounded-lg focus:ring-primary-500 focus:border-primary-500 font-bold">
                ...
                <div class="col-span-3">
                    ...
                    <input type="text" name="subject" value="{{ old('subject', $invoice->subject) }}"
                        class="w-full px-4 py-2 border rounded-lg focus:ring-primary-500 focus:border-primary-500"
            ...
            <div class="grid grid-cols-2 gap-6">
                <div>
                    ...
                    <textarea name="notes" rows="3"
                        class="w-full px-4 py-2 border rounded-lg focus:ring-primary-500 focus:border-primary-500"
                        placeholder="Catatan untuk client">{{ old('notes', $invoice->notes) }}</textarea>
                </div>
                <div>
                    <label class="block text-sm font-medium text-gray-700 mb-1">Syarat & Ketentuan</label>
                    <textarea name="terms" rows="3"
                        class="w-full px-4 py-2 border rounded-lg focus:ring-primary-500 focus:border-primary-500"
                        placeholder="Syarat pembayaran">{{ old('terms', $invoice->terms) }}</textarea>
                </div>
            </div>
        </div>
                ...
                <div>
                    <label class="inline-flex items-center">
                        <input type="checkbox" name="include_signature" value="1" 
                            {{ old('include_signature', $invoice->include_signature) ? 'checked' : '' }}
                ...
                <div>
                    <label class="inline-flex items-center">
                        <input type="checkbox" name="include_stamp" value="1"
                            {{ old('include_stamp', $invoice->include_stamp) ? 'checked' : '' }}
                            class="rounded border-gray-300 text-primary-600 shadow-sm focus:border-primary-300 focus:ring focus:ring-primary-200 focus:ring-opacity-50">
                ...
                <div>
                    <label class="inline-flex items-center">
                        <input type="checkbox" name="include_qr" value="1"
                            {{ old('include_qr', $invoice->include_qr) ? 'checked' : '' }}
                            class="rounded border-gray-300 text-primary-600 shadow-sm focus:border-primary-300 focus:ring focus:ring-primary-200 focus:ring-opacity-50">
        ...
        <div class="flex justify-end gap-3">
            <a href="{{ route('invoices.show', $invoice) }}" class="px-6 py-2 border rounded-lg hover:bg-gray-50">Batal</a>
            <button type="submit" class="px-6 py-2 bg-primary-600 text-white rounded-lg hover:bg-primary-700">Simpan Perubahan</button>
        </div>
    </form>
        ...
        <script>
            function invoiceForm() {
                return {
                    ...
                    addItem() {
                        this.items.push({ id: null, description: '', quantity: 1, unit: 'pcs', unit_price: 0 });
                    },
