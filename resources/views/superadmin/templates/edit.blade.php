...
@section('title', 'Edit Template Sistem')
@section('header', 'Edit Template')

@section('content')
    <div class="max-w-4xl mx-auto">
        <div class="bg-white rounded-xl shadow-sm border border-gray-100 p-6">
            <form action="{{ route('superadmin.templates.update', $template) }}" method="POST" class="space-y-6">
                @csrf
                @method('PUT')
                ...
                <div class="grid grid-cols-1 md:grid-cols-2 gap-6">
                    ...
                    <div>
                        ...
                        <input type="url" name="thumbnail_url" value="{{ old('thumbnail_url', $template->thumbnail) }}"
                            class="w-full px-4 py-2 border rounded-lg focus:ring-primary-500 focus:border-primary-500"
                ...
                <div>
                    ...
                    <div class="border rounded-lg overflow-hidden">
                        <textarea name="html_content" id="html_content" rows="20"
                            class="w-full px-4 py-2 font-mono text-sm focus:ring-primary-500 focus:border-primary-500">{{ old('html_content', $template->html_content) }}</textarea>
                    </div>
